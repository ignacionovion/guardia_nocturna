    ...
    <div class="relative">
        ...
        <select {{ $attributes->merge([
            'class' => 'w-full rounded-xl border text-sm transition-all duration-150 appearance-none cursor-pointer ' .
                ...
                ' text-slate-900 dark:text-white focus:outline-none focus:ring-4 focus:bg-white dark:focus:bg-slate-900'
        ]) }}>
            {{ $slot }}
        </select>

        <div class="absolute inset-y-0 right-0 pr-3.5 flex items-center pointer-events-none">
            <i class="fas fa-chevron-down text-slate-400 dark:text-slate-500 text-xs"></i>
        </div>
    </div>
