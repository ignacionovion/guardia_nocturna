...
@section('title', 'Reemplazos - ' . branding()->nombre_empresa)
...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm">
        <form action="{{ route('admin.reports.replacements') }}" method="GET" class="flex flex-wrap items-end gap-4">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.reports.replacements') }}" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-700 dark:text-slate-300 font-extrabold text-xs uppercase tracking-widest transition-all" title="Limpiar filtros">
    ...
    {{-- STATS CARDS --}}
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-4 mb-6 rounded-lg">
        @php
            $statCards = [
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-5 lg:col-span-2">
            <div class="flex items-center justify-between mb-4">
                <div class="text-sm font-black text-slate-800 dark:text-white uppercase tracking-wide">Reemplazos (por día)</div>
    ...
    {{-- Por Guardia --}}
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-5 mb-6">
        <div class="flex items-center justify-between mb-4">
        ...
        <div class="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
            @foreach($guardiaLabels as $idx => $label)
                <div class="flex items-center justify-between bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2">
                    <div class="text-sm font-bold text-slate-700 dark:text-slate-300">{{ $label }}</div>
            ...
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50 dark:bg-slate-800">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Guardia</th>
                        <th class="px-6 py-3 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Reemplazado</th>
                        <th class="px-6 py-3 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Reemplazante</th>
                        <th class="px-6 py-3 text-center text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Estado</th>
                    </tr>
                </thead>
