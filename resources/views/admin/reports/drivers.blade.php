...
<div class="container mx-auto px-4 py-6 max-w-7xl">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-t-lg border border-slate-200 dark:border-slate-700">
        <div class="flex overflow-x-auto">
            ...
            <a href="{{ route('admin.reports.refuerzos') }}" class="flex items-center gap-2 px-6 py-4 text-sm font-semibold whitespace-nowrap border-b-2 border-transparent text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:text-white transition-colors">
    ...
    <div class="bg-white dark:bg-slate-900 p-5 border border-t-0 border-slate-200 dark:border-slate-700 mb-6 rounded-b-lg shadow-sm">
        <form action="{{ route('admin.reports.drivers') }}" method="GET" class="flex flex-wrap items-end gap-4">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.reports.drivers') }}" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-700 dark:text-slate-300 font-extrabold text-xs uppercase tracking-widest transition-all" title="Limpiar filtros">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50 dark:bg-slate-800">
                    <tr>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Conductor</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Guardia</th>
                        <th scope="col" class="px-6 py-3 text-center text-xs font-bold text-emerald-600 uppercase tracking-wider">Turnos presentes</th>
                        <th scope="col" class="px-6 py-3 text-center text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider">Días únicos</th>
                    </tr>
                </thead>
                        ...
                        <tr class="hover:bg-slate-50 dark:bg-slate-800 transition-colors">
                            <td class="px-6 py-3 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="text-sm font-bold text-slate-700 dark:text-slate-300">{{ $row['name'] ?? '' }}</div>
                                </div>
                            </td>
                            <td class="px-6 py-3 whitespace-nowrap text-sm font-bold text-slate-700 dark:text-slate-300">{{ $row['guardia'] ?? '' }}</td>
                            <td class="px-6 py-3 whitespace-nowrap text-center text-sm font-black text-emerald-700">{{ $row['present_shifts'] ?? 0 }}</td>
                            <td class="px-6 py-3 whitespace-nowrap text-center text-sm font-bold text-slate-700 dark:text-slate-300">{{ $row['unique_days'] ?? 0 }}</td>
                        </tr>
                    @empty
                        <tr>
