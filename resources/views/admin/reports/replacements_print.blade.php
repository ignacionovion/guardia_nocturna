...
@section('content')
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-6 mb-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-black text-slate-800 dark:text-white uppercase tracking-tight">Reporte de Reemplazos</h1>
                <div class="text-sm text-slate-500 dark:text-slate-400 font-medium mt-1">{{ $kpis['range_label'] ?? '' }}</div>
            ...
            <div class="text-right">
                ...
                <div class="text-sm font-bold text-slate-700 dark:text-slate-300">{{ now()->format('d-m-Y H:i') }}</div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-6">
            <div class="border border-slate-200 dark:border-slate-700 rounded-lg p-4">
                ...
                <div class="text-2xl font-black text-slate-800 dark:text-white mt-1">{{ $kpis['total_replacements'] ?? 0 }}</div>
            </div>
            <div class="border border-slate-200 dark:border-slate-700 rounded-lg p-4">
                <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Reemplazantes únicos</div>
                <div class="text-2xl font-black text-slate-800 dark:text-white mt-1">{{ $kpis['unique_replacers'] ?? 0 }}</div>
            </div>
            <div class="border border-slate-200 dark:border-slate-700 rounded-lg p-4">
                <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Reemplazados únicos</div>
                <div class="text-2xl font-black text-slate-800 dark:text-white mt-1">{{ $kpis['unique_replaced'] ?? 0 }}</div>
            </div>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200">
                ...
                <tbody class="bg-white dark:bg-slate-900 divide-y divide-slate-100">
                    @forelse($events as $e)
                        <tr>
                            <td class="px-6 py-2 whitespace-nowrap text-sm font-bold text-slate-700 dark:text-slate-300">{{ optional($e->start_date)->format('d-m-Y H:i') }}</td>
                            <td class="px-6 py-2 whitespace-nowrap text-sm font-bold text-slate-700 dark:text-slate-300">{{ optional($e->end_date)->format('d-m-Y H:i') ?? '—' }}</td>
                            <td class="px-6 py-2 whitespace-nowrap text-sm text-slate-600 dark:text-slate-400">{{ $e->firefighter?->guardia?->name ?? $e->user?->guardia?->name ?? 'Sin Asignar' }}</td>
                            <td class="px-6 py-2 whitespace-nowrap text-sm text-slate-700 dark:text-slate-300">
                                {{ $e->firefighter
