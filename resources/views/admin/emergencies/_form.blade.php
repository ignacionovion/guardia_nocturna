    ...
    <div class="lg:col-span-2">
        <div class="card-base shadow-sm overflow-visible">
            ...
            <div class="p-5">
                <div class="space-y-4">
                    <div>
                        <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">Clave</label>
                        ...
                        <button type="button" id="btn-pick-key" class="w-full text-left px-4 py-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800 hover:bg-white dark:hover:bg-slate-700/50 transition">
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <div class="text-xs text-slate-500 dark:text-slate-400 mt-0.5" id="key-desc">{{ $selectedKeyId ? \Illuminate\Support\Str::limit(optional($keys->firstWhere('id', (int)$selectedKeyId))->description, 60) : 'Usa el buscador para encontrar la clave.' }}</div>
                                </div>
                                <i class="fas fa-magnifying-glass text-slate-400 mt-1"></i>
                            </div>
                        </button>

                        <div class="text-[11px] text-slate-500 dark:text-slate-400 mt-1">Búsqueda rápida (por código o descripción).</div>
                    </div>
                        ...
                        <button type="button" id="btn-pick-units" class="w-full text-left px-4 py-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800 hover:bg-white dark:hover:bg-slate-700/50 transition">
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <div class="text-xs text-slate-500 dark:text-slate-400 mt-0.5"><span id="units-count">{{ count($selectedUnitIds) }}</span> seleccionada(s)</div>
                                </div>
                                <i class="fas fa-magnifying-glass text-slate-400 mt-1"></i>
                            </div>
                        </button>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">A cargo</label>
                                ...
                                <div id="officer-options-list" class="py-1">
                                    <div class="officer-option px-3 py-2.5 hover:bg-white dark:hover:bg-slate-800 cursor-pointer transition-colors flex items-center gap-3 {{ !$selectedOfficerId ? 'bg-blue-50' : '' }}"
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <div class="text-sm font-medium text-slate-700 dark:text-slate-300">Sin asignar</div>
                                        </div>
                                    </div>
                                    @foreach($onDutyUsers as $u)
                                        <div class="officer-option px-3 py-2.5 hover:bg-white dark:hover:bg-slate-800 cursor-pointer transition-colors flex items-center gap-3 {{ (string)$selectedOfficerId === (string)$u->id ? 'bg-blue-50' : '' }}"
                                            ...
                                            <div class="flex-1 min-w-0">
                                                ...
                                                @if($u->cargo_texto)
                                                    <div class="text-xs text-slate-500 dark:text-slate-400">{{ $u->cargo_texto }}</div>
                                                @endif
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                                <div id="officer-no-results" class="hidden px-3 py-4 text-center text-xs text-slate-500 dark:text-slate-400">
                                    No se encontraron resultados
                                </div>
                            </div>
                        </div>
                        
                        <div class="text-[11px] text-slate-500 dark:text-slate-400 mt-1">Solo personal en servicio (turno activo).</div>
                    </div>
                </div>
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-white dark:bg-slate-800 sticky top-0">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Acción</th>
                    </tr>
                </thead>
...
@foreach($units as $u)
                    ...
                    <label class="unit-item flex items-start gap-3 p-3 rounded-lg border {{ $unitActive ? 'border-slate-200 dark:border-slate-700 hover:bg-white dark:hover:bg-slate-800 cursor-pointer' : 'border-red-200 bg-red-50/60 opacity-70 cursor-not-allowed pointer-events-none' }} transition" data-name="{{ strtolower($u->name) }}" data-desc="{{ strtolower($u->description ?? '') }}">
                        ...
                        <div class="flex-1">
                            ...
                            @if($u->description)
                                <div class="text-xs text-slate-500 dark:text-slate-400">{{ $u->description }}</div>
                            @endif
                            @if(!$unitActive)
