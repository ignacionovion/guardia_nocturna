...
<div class="container mx-auto px-4 py-6">
    <div class="mb-6">
        ...
        <p class="text-slate-600">Personaliza la apariencia de GuardiAPP con los colores y logo de tu compañía.</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-6">
            ...
            <div class="mb-4">
                @if($branding?->logo_path)
                    <div class="bg-slate-50 rounded-lg p-4 flex items-center justify-center mb-4">
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-6">
            ...
            <form action="{{ route('admin.branding.upload-favicon') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-3">
                    <div>
                        <label for="favicon" class="block text-sm font-medium text-slate-700 mb-1">
                            Subir nuevo favicon
                        </label>
                        <input type="file" name="favicon" id="favicon" accept=".png,.ico"
                            class="w-full text-sm text-slate-600 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-amber-50 file:text-amber-700 hover:file:bg-amber-100">
                        <p class="text-xs text-slate-500 mt-1">Formatos: PNG, ICO. Máximo 1MB.</p>
                        @error('favicon')
                            <p class="text-rose-600 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <form action="{{ route('admin.branding.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div class="md:col-span-2">
                        <label for="nombre_empresa" class="block text-sm font-medium text-slate-700 mb-1">
                        ...
                        <p class="text-xs text-slate-500 mt-1">Este nombre aparecerá en el header del sistema.</p>
                    ...
                    {{-- Primary Color --}}
                    <div>
                        <label for="color_primario" class="block text-sm font-medium text-slate-700 mb-1">
                            Color Primario
                        </label>
                        <div class="flex items-center gap-3">
                            <input type="color" name="color_primario" id="color_primario"
                                ...
                                class="h-10 w-20 rounded border-slate-300 cursor-pointer">
                            <input type="text" 
                                value="{{ old('color_primario', $branding?->color_primario ?? $defaults['color_primario']) }}"
                                class="flex-1 rounded-lg border-slate-300 text-sm font-mono uppercase"
                                readonly>
                        </div>
                        <p class="text-xs text-slate-500 mt-1">Usado en botones y elementos principales.</p>
                    ...
                    {{-- Secondary Color --}}
                    <div>
                        <label for="color_secundario" class="block text-sm font-medium text-slate-700 mb-1">
                            Color Secundario
                        </label>
                        <div class="flex items-center gap-3">
                        ...
                        <p class="text-xs text-slate-500 mt-1">Usado en elementos de contraste.</p>
                    ...
                    {{-- Sidebar Color --}}
                    <div>
                        <label for="color_sidebar" class="block text-sm font-medium text-slate-700 mb-1">
                            Color del Sidebar
                        </label>
                        <div class="flex items-center gap-3">
                        ...
                        <p class="text-xs text-slate-500 mt-1">Color de fondo del menú lateral.</p>
                        @error('color_sidebar')
                            <p class="text-rose-600 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="bg-slate-50 rounded-lg p-4 mb-6">
                    <h3 class="text-sm font-medium text-slate-700 mb-3">Vista previa</h3>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <a href="{{ route('dashboard') }}" class="text-slate-600 hover:text-slate-800">
                        Cancelar
                    </a>
                </div>
            </form>
