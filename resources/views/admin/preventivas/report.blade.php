...
<div class="w-full">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div>
            <div class="text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">Preventivas</div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4 mb-6">
        ...
        <!-- Tarjetas de Tipos -->
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-4 shadow-sm border-l-4 border-l-slate-500">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-4 shadow-sm border-l-4 border-l-purple-500">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-4 shadow-sm border-l-4 border-l-sky-500">
                            ...
                            <table class="w-full text-sm">
                                <thead>
                                    <tr class="border-b border-slate-200 dark:border-slate-700">
                                        <th class="text-left py-2 px-3 font-black text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400">Bombero</th>
                                        <th class="text-left py-2 px-3 font-black text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400">RUT</th>
                                        <th class="text-left py-2 px-3 font-black text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400">Cargo</th>
                                        <th class="text-left py-2 px-3 font-black text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400">Tipo</th>
                                        <th class="text-left py-2 px-3 font-black text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400">Reemplaza a</th>
                                        <th class="text-left py-2 px-3 font-black text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400">Hora Entrada</th>
                                        <th class="text-left py-2 px-3 font-black text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400">Asistencia</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($shift->assignments->sortByDesc('attendance') as $assignment)
                                        ...
                                        @endphp
                                        <tr class="border-b border-slate-100 dark:border-slate-800 last:border-0 {{ $hasAttendance ? 'bg-emerald-50/30' : ($fueReemplazado ? 'bg-rose-50/30' : '') }}">
                                            <td class="py-3 px-3">
                                                <div class="font-bold {{ $fueReemplazado ? 'text-slate-400 line-through' : 'text-slate-900' }}">{{ $f?->apellido_paterno ?? 'N/A' }}</div>
                                                <div class="text-xs {{ $fueReemplazado ? 'text-slate-300' : 'text-slate-500 dark:text-slate-400' }}">{{ $f?->nombres ?? '' }}</div>
                                            </td>
                                            <td class="py-3 px-3 text-slate-600 dark:text-slate-400 font-mono text-xs {{ $fueReemplazado ? 'text-slate-400' : '' }}">{{ $f?->rut ?? 'N/A' }}</td>
                                            <td class="py-3 px-3 {{ $fueReemplazado ? 'text-slate-400' : 'text-slate-600 dark:text-slate-400' }}">{{ $f?->cargo_texto ?? '-' }}</td>
                                            <td class="py-3 px-3">
                                            ...
                                            <td class="py-3 px-3 {{ $fueReemplazado ? 'text-slate-400' : 'text-slate-600 dark:text-slate-400' }}">
