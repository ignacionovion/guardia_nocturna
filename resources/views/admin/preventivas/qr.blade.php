...
<div class="container mx-auto px-4 py-8">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            ...
            <div class="text-sm text-slate-600 dark:text-slate-400 mt-1">Este QR abre el formulario del turno actual.</div>
        </div>

        <div class="flex items-center gap-2">
            <a href="{{ route('admin.preventivas.show', $event) }}" class="px-4 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold text-xs">Volver</a>
        </div>
    </div>
    ...
    <div class="mt-6 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
            ...
            <div>
                <div class="text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">Link</div>
                <div class="mt-2 break-all text-sm font-semibold text-slate-800 dark:text-white">{{ $url }}</div>
                ...
                @if($status !== 'closed')
                    <form method="POST" action="{{ route('admin.preventivas.qr.regenerate', $event) }}" class="mt-3" onsubmit="return confirm('Esto generará un nuevo QR/link y el anterior dejará de funcionar. ¿Continuar?');">
                        @csrf
                        <button type="submit" class="inline-flex items-center gap-2 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:bg-slate-800 text-slate-900 font-black py-3 px-6 rounded-xl text-[11px] transition-all shadow-md hover:shadow-lg uppercase tracking-widest border border-slate-200 dark:border-slate-700">
