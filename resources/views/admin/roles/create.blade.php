...
@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-3xl font-bold text-slate-800 dark:text-white">Nuevo Rol</h1>
                <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Crea un rol y define permisos por secciones</p>
            </div>
            <a href="{{ route('admin.roles.index') }}" class="inline-flex items-center text-slate-600 dark:text-slate-400 hover:text-slate-900">
            ...
            <form method="POST" action="{{ route('admin.roles.store') }}" class="p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-4 flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                                    @foreach(['guardias', 'dotaciones', 'calendario', 'camas'] as $key)
                                        ...
                                        <label class="group flex items-center gap-3 p-3 rounded-xl border-2 border-slate-200 dark:border-slate-700 hover:border-blue-300 hover:bg-blue-50/30 cursor-pointer transition-all">
                                            ...
                                            <span class="font-semibold text-slate-700 dark:text-slate-300 group-hover:text-slate-900">{{ $label }}</span>
                                        </label>
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                                    @foreach(['voluntarios', 'usuarios', 'roles'] as $key)
                                        @php [$label, $icon, $colorClass] = $sections[$key]; @endphp
                                        <label class="group flex items-center gap-3 p-3 rounded-xl border-2 border-slate-200 dark:border-slate-700 hover:border-emerald-300 hover:bg-emerald-50/30 cursor-pointer transition-all">
                                            <input type="checkbox" name="permissions[]" value="{{ $key }}"
                                                class="w-5 h-5 rounded border-slate-300 dark:border-slate-600 text-emerald-600 focus:ring-emerald-500"
                                                {{ in_array($key, $selected, true) ? 'checked' : '' }}>
                                            <div class="w-8 h-8 rounded-lg {{ $colorClass }} flex items-center justify-center text-sm">
                                                <i class="fas {{ $icon }}"></i>
                                            </div>
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                                    @foreach(['dashboard', 'emergencias', 'reportes', 'admin_system'] as $key)
                                        @php [$label, $icon, $colorClass] = $sections[$key]; @endphp
                                        <label class="group flex items-center gap-3 p-3 rounded-xl border-2 border-slate-200 dark:border-slate-700 hover:border-slate-400 hover:bg-slate-50 dark:bg-slate-800 cursor-pointer transition-all">
                                            <input type="checkbox" name="permissions[]" value="{{ $key }}"
                ...
                <div class="mt-8 flex items-center justify-end gap-3">
                    <a href="{{ route('admin.roles.index') }}" class="px-5 py-2.5 rounded-xl border-2 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:bg-slate-800 font-semibold transition-colors">Cancelar</a>
