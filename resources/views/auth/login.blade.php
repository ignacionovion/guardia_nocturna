...
@section('title', 'Acceso - '.config('app.name', 'EstacionAPP'))
        ...
        <div class="tenant-auth-wrap">
            ...
            <div class="tenant-auth-card">
                <div class="tenant-auth-body">
                    <h1 class="tenant-auth-title">Bienvenido</h1>
                    <p class="tenant-auth-subtitle">Ingresa tus credenciales para acceder al sistema.</p>

                    <form method="POST" action="{{ url()->current() }}" class="tenant-auth-form">
                        @csrf

                        <div class="tenant-auth-field">
                            <label for="username" class="tenant-auth-label">Usuario</label>
                            <div class="tenant-auth-input-wrap">
                                <i class="fas fa-user tenant-auth-icon"></i>
                                <input
                                    id="username"
                                    type="text"
                                    name="username"
                                    ...
                                    class="tenant-auth-input @error('username') tenant-auth-input-error @enderror"
                                >
                            </div>
                            @error('username')
                        ...
                        <div class="tenant-auth-field">
                            <label for="password" class="tenant-auth-label">Contrasena</label>
                            <div class="tenant-auth-input-wrap">
                                <i class="fas fa-lock tenant-auth-icon"></i>
                                <input
                                    id="password"
                                    type="password"
                                    name="password"
